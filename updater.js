...
(async () => {
  const { data: versions } = await axios.get('https://raw.githubusercontent.com/Priyansh-11/Priyansh-bot/main/versions.json');
  ...
  for (const filePath in files) {
    ...
    let getFile;
    try {
      const response = await axios.get(`https://github.com/Priyansh-11/Priyansh-bot/raw/main/${filePath}`, {
  ...
  const { data: packageHTML } = await axios.get("https://github.com/Priyansh-11/Priyansh-bot/blob/main/package.json");
