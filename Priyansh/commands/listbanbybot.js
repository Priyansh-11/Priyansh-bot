    ...
    case "-t":
      {
        ...
        return api.sendMessage(listBanned.length != 0 ? api.sendMessage(`Currently ${listBanned.length} Groups Having Bot Banned\n\n${listBanned.join("\n")}` +
          "\n\nReply to message + order number -> unban",
          threadID, (error, info) => {
          ...
          messageID
        ) : "There are currently no banned groups!", threadID, messageID);
      }
    case "user":
    case "u":
    case "-u":
      {
        ...
        return api.sendMessage(listBanned.length != 0 ? api.sendMessage(`Currently ${listBanned.length} Users Having Bot Banned\n\n${listBanned.join("\n")}` +
          "\n\nReply to message + order number -> unban",
          threadID, (error, info) => {
